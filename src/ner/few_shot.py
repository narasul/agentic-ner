...
from ner.prompts import SYSTEM_PROMPT_FOR_JSON_OUTPUT
...
class NERMaster:
    ...
    def recognize(self, tokens: List[str]) -> List[str]:
        ...
        llm_output = self.llm_client.get_llm_response(
            query, SYSTEM_PROMPT_FOR_JSON_OUTPUT
        )
    ...
    def convert_to_genia_labels(
        ...
        json_string = NERMaster._extract_json(llm_output).replace("\\n", "")
        llm_output_json = {"DNA": [], "RNA": [], "protein": [], "cell_type": []}
        try:
            llm_output_json = json.loads(json_string.strip())
        except Exception as err:
            print(f"Error while parsing json: {str(err)}")

        entities = []
        for entity_type in ["DNA", "RNA", "protein", "cell_type"]:
            for entity in llm_output_json[entity_type]:
                ...
                if start_pos != -1:
                    entities.append(
                        {
                            "start": start_pos,
                            "end": end_pos + 1 if end_pos != -1 else start_pos + 1,
                            "type": entity_type,
                        }
                    )
        return entities
